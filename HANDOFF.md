...
|---|---|---|---|
| `index.html` | 2,461,518 | `22e3566d68a003841a6003f539c3bf7aef1caafde7782abc9e2b467bbd518fed` | **🟢 `35m` / Amethyst `#7A5A98` — RESUME OR START FRESH (§158). SHIPPED s69 on this hash; battery run on HIS MACHINE (by Claude through his shell) before the ship. Prev state `35l` / Magenta `#A8478F` (commit `4b2d982`, live `acca42c6…`): battery PASSED on his machine 19 Sep 15:37 on `acca42c6` — STATIC clean, BEHAVIOUR 65/65, SESSION 21/21 (ran just after the ship; covers the live build). 🟢 CODE-REVIEW FIXES (§157): four defects the two code-review seats found in THIS session's own new code, fixed. (1) §149 autosave restore leaked a PERSONAL draft into a Desk/curator build (`startBuild` restored unconditionally while `saveDraft` was gated) → could misfile personal clues as an archive case + wipe the draft; restore now gated on `!_curatorBuildFlow && !_curatorEdit`, and the `_finishBuild` `clearDraft` gated so a Desk/edit finish can't wipe a personal draft. Curator/owner-only. (2) §154 a timed auto-file firing while the submit panel was open left a stuck dialog and could re-run finish; `autoFile` now dismisses it via `agcNo()` and `finishHunt` re-checks `State.locked` after the panel. (3) §150 bundle button gated on only the volume's first code → now `ownsVolume(_bv.id)` (all codes). (4) §156 clue counter stale on a prefilled/cleared field → `clueCount` called in `openTileEdit` and `openCrop`. §152 (seat cap) and §155 (review) reviewed CLEAN. STATIC green, `35l`, 117 handlers, no drift. FULL BATTERY OWED. Prev state `35k` / Verdigris `#4E9A87` (SHIPPED, commit `a8b9fbf`, live `28617fa2`): 🟢 REVIEW ERGONOMICS + CLUE COUNTER (§155/§156): (§155) the verify ✓/✗ now split the FULL tile height (`.vtoggle{align-self:stretch}` + `.vt{flex:1;min-height:44px}`); a "Confirm all remaining" ghost button confirms every FOUND-and-unjudged tile at once (never a NOT-FOUND tile); and a judged tile wears an on-brand stamp — CONFIRMED (green) / RETURNED (oxblood) — reusing the EXACT `.tile.found` SOLVED-stamp treatment via `.vthumb.jstamp::after`. NOTHING HIDDEN; every verdict stays revisable (re-tapping restamps). Owner ruling: don't hide judged tiles, stamp them. (§156) both clue inputs (`#crop-clue`, `#tile-clue`) get a live "N/12" counter that turns oxblood at the limit and brightens the existing "+tap here to add hidden hint" line (`.at-limit`); the 12-char cap is unchanged. STATIC green, `35k`, 117 handlers resolved, no drift. FULL BATTERY OWED; after ship verify the stamp + confirm-all on the live review screen and the counter in the builder. Prev state `35j` / Amethyst `#7A5A98` (SHIPPED, commit `fd3925b`, live `a0c8b706`): 🟢 SUBMIT PANEL (§154): the hunt submit/lock confirm is now the Agency's own on-brand overlay `ov-agconfirm` (modelled on `ov-buy`), NOT the browser's native `confirm()` — no more "gahensley1.github.io says". Fires on BOTH partial and full finish (native fired only on partial). `agencyConfirm({eyebrow,title,bodyHTML,cta,plain})` → Promise<bool>; primary=true, ghost/backdrop=false (backdrop routed through `agcNo`, never orphans the promise); falls back to `window.confirm(plain)` if the overlay is missing. Owner-approved copy, centered, numerals: eyebrow "CASE No. <code>", title "Present your findings?"/"A clean sweep?", "<n> of <N> cracked." on its own line, "Once filed, it is sealed." in oxblood, CTA "File my findings"/"File the case". ALSO carries §153 (the staged `isStandalone()` helper, inert until wired to the push-availability line). STATIC green, `35j`, 115 handlers resolved, no drift. FULL BATTERY OWED; after ship Chrome-verify the panel appears, submits, and cancels. Prev state `35i` / Rose `#B5566B` (SHIPPED, commit `b12768e`, live `fff46d57`): 🟢 SEAT-RACE HARD CAP (§152): the join complement is now enforced ATOMICALLY on the Worker (v2.6.18, DEPLOYED + live-verified) via a new `seat:CODE:HID` row and a single count-and-insert statement — two hunters can no longer both take the last seat. Live burst: 8 simultaneous claims for 3 seats → exactly 3 seated, 5 refused; resume force-seats; idempotent; test seats cleaned up. Client `joinCase` calls `/claim-seat`; offline falls back to the old local seat. Worker `worker-v2_6_18.js` (gitignored, on disk) — owner deployed it via the Cloudflare dashboard; Claude never held Worker access (SSA.1). STATIC green, `35i`, no drift. FULL BATTERY OWED before ship. Prev state `35h` / Ochre `#C88A2E` (SHIPPED, commit `21036a9`, live `18e0ba42`): 🟢 BUNDLE SURFACED (§150): the $8.99 Charter+Almanac-I bundle now appears on the licence screen, stacked beneath the Charter button and wired to `buyEverything()`; shown only when the Charter isn't already held and the bundle volume isn't owned (`bundleVolume()` + `ownsPack`). Owner copy VERBATIM: "The Inspector's Charter & The Almanac, Volume I" with plain subtitle "unlimited builds + 3 ready-made cases — $8.99". §149 (builder autosave + EU-push comment) already SHIPPED SEPARATELY as `35g` (commit `ec65bee`, live `7aabc352`, verified — Pages buildmark `35g`, `sw.js` EU note live); `35h` adds ONLY the bundle button on top of it. STATIC green, `35h`, no drift, 113 handlers all resolved; BATTERY PASSED ON HIS MACHINE on this exact hash (BEHAVIOUR 65/65, SESSION 21/21). After ship, Chrome-verify the licence screen shows both buttons. Prev state `35g` / Cobalt `#3B6BA5` (SHIPPED, commit `ec65bee`, live `7aabc352`): 🟢 BUILDER AUTOSAVE + EU-PUSH COMMENT (§149): a fresh, unfiled build (photos and all) now persists to IndexedDB and is restored SILENTLY on the next builder open — the completion seat's biggest finding (a half-built case could be lost, no net). FRESH BUILDS ONLY (`code===null`, never `_builderEdit`/`_curatorEdit`/`_curatorBuildFlow`); IDB not localStorage (photo boards run to MB, past quota); saved on `renderBuild` + a 4 s builder-visible interval + `visibilitychange`/`pagehide`; cleared on file (once recorded in `mine[]`) and on confirmed leave. Restore is SILENT — no prompt, so no invented in-product copy; a "resume vs start fresh" prompt is available if the owner supplies wording. Best-effort: no IDB → old behaviour, no harm. Also: the stale "EU push unavailable" note in `sw.js` corrected (Platform seat — Apple reversed that in 2024). SW CACHE unchanged (no cached asset changed; `index.html` is network-first). STATIC green, `35g`, no drift. FULL BATTERY OWED ON HIS MACHINE before ship; after ship, Chrome-verify build→reload→restore on a real board. Prev state `35f` / Rust `#B4532A`: 🟢 PRICING (§148): Charter $6.99→$7.99 (all 3 shown spots); bundle `BUNDLE_PRICE` 10.99→8.99; Almanac Vol I flagged `bundle:true` so `bundleVolume()` resolves. ⚠ THE BUNDLE IS SURFACED NOWHERE — `buyEverything()` is defined but uncalled and `renderLicTiers` sells only the Charter (its own comment says so); the price is corrected for whenever it is surfaced, which needs owner copy. Volumes unchanged, $1.49 each (3 packs = $4.47). STATIC green, `35f`, no drift. FULL BATTERY OWED ON HIS MACHINE before ship. Prev state `35e` / Lime `#7FA33C`: 🟢 TOUR-CIPHER GUARD + 4 REVIEW FIXES (§147). STATIC green, `35e`, no drift. Chrome-verified: tour defers while `#cred-ov` is open; dispatch/praise backdrops route through burn; coldSample retry present. `35d` (pub sign back to original) WAS the state before. 🔴 PUB SIGN BACK TO ORIGINAL (§146.3). Owner not happy with the black-on-white cutout (`35c`, shipped) either — restored the ORIGINAL rendered gold-on-green sign byte-exact (sha `86e4df1d`, 27,250 B). SW CACHE `shco-v7`→`shco-v8`. STATIC green, `35d`. THE PUB-SIGN ART IS SETTLED AS THE ORIGINAL; the swaps were `35a` gold-brass (bad on phone), `35c` black-white cutout (owner disliked), both reverted. 🟢 PUB SIGN = BLACK-ON-WHITE, CUT OUT (§146.2). After the gold `35a` failed the phone, the owner chose a weathered black-on-white enamel street nameplate — **contrast 12.59:1**, nearly 3× the others, clears AAA and the outdoor 7:1 target. Fitted 720×192, then BACKGROUND CUT OUT: border-flood removed the outer white, the black frame protecting the white sign face (2,305 transparent px, rounded corners like the original). Written over `67436406c2.webp`; SW CACHE `shco-v6`→`shco-v7`. STATIC green, `35c`. `35b` (reverted original) WAS the state before this. 🔴 PUB SIGN REVERTED (§146.1): the weathered brass sign (shipped in `35a`) DID NOT READ ON THE PHONE — the owner reverted. Original rendered sign restored byte-exact (sha `86e4df1d`), SW CACHE `shco-v5`→`shco-v6` so players re-fetch it. STATIC green, `35b`, no drift. `35a` (weathered sign) WAS LIVE at `3f31fe34`, commit reverted. 🟢 PUB SIGN SWAPPED (§146): the welcome-tour sign `assets/img/67436406c2.webp` replaced with the owner's weathered brass-and-gold enamel (27 KB→45 KB, same 720×192 dims, filename unchanged so no markup edit). SW CACHE `shco-v4`→`shco-v5` so returning players fetch it. STATIC green, `35a`, no drift. `34z` (Ochre, portrait fix) was LIVE. 🟢 ASSET EXTRACTION PHASE 2 + A REGRESSION FIX (§143, §144). `34y` (Cobalt) was Phase 2 BUT SHIPPED NOTHING — it blanked every detective portrait (§144) and was caught in preview before ship; `34z` is the fixed build. STATIC green, buildmark `34z`, no drift. FULL BATTERY OWED; after deploy probe the 58 asset URLs. 🟢 ASSET EXTRACTION PHASE 2 (§143): the 32 JS-CONSTANT images (coins, portraits, badge, seal, paw) pulled OUT to `assets/img/` too. Document now **2.45 MB** (was 4.40 at s68 open). 58 asset files, 1.40 MB. STATIC green, buildmark `34y`, no drift. Canvas composites (portraits/coins/badge) draw these via `await new Promise(onload)` + same-origin, so file URLs are safe by guarantee (§143.2). FULL BATTERY OWED; after deploy probe the (now 58) asset URLs. `34x` was BUILT then — see below. The gazetteer (~973 KB) and font are STILL INLINE — Phase 3 only. 🟢 ASSET EXTRACTION PHASE 1 (§142): the 26 `<img>`/CSS chrome images pulled OUT of the document into `assets/img/` (committed, Pages-served), cached-first by `sw.js`. Document 4.40 MB → 3.14 MB, and base64→binary trims the 33% tax. **PIXEL-IDENTICAL BY CONSTRUCTION** — image bytes untouched, only delivery changed; round-trip re-encode asserted equal. STATIC green in the sandbox, buildmark `34x`, no drift. FULL BATTERY OWED ON HIS MACHINE before ship; after deploy the 26 asset URLs must be probed for 200. `34w` / Lime IS LIVE at `9d74390f`, commit `064f1a28` (the join on-ramp, §141). The gazetteer (`GAZ5_BLOB`, 506 KB) and the woff2 font are STILL INLINE — Phases 2/3, not done. The join-screen on-ramp to the free Agency cases (§141.4/.5), owner copy "No case number? Inspect the Agency's own cases." STATIC green in the sandbox; FULL BATTERY OWED ON HIS MACHINE before ship. `34v` was LIVE at `dd6be1e5`, commit `6cc826d8`. `SHARED_MAX_BYTES` 2,000,000 → 3,900,000 to match Worker **v2.6.15** (§138). Full battery PASSED ON HIS MACHINE on this exact hash BEFORE the ship — STATIC clean, BEHAVIOUR 65/65, SESSION 21/21, and **Claude read the whole log himself off `test\.last-battery.log` (§134), no pasting.** Verified against the COMMIT SHA: same hash, buildmark `34v`, `SHARED_MAX_BYTES = 3900000` in the committed file.**<br>`34u` shipped at s67, commit `6ec02bd5`, hash `97c75302c254719c662985962a687fcf8e81b73419c19e7239b2a24a1e728ac5`. |
| `sw.js` | 5,532 | `7c0074f033a40959b174cb85dbf6a5c9bbaa134156dbc2d13040c160c54e8a0e` | **⚠ BUILT with `35d` — CACHE `shco-v7`→`shco-v8` (§146.3, pub sign back to original).** |
...
# 🟢 §158 — RESUME OR START FRESH. `35m`, s69.

Owner approved the prompt "Resume or start fresh?" (s69). When Case Builder opens and an unfiled draft (§149) exists, `startBuild` now asks via `agencyConfirm` BEFORE the builder opens: **Resume** (primary) restores the board; **Start fresh** calls `clearDraft()` and opens an empty board. Button labels taken from the owner's own phrase. **A backdrop tap RESUMES** — a stray tap never discards work. To allow that, `agencyConfirm` gained two optional fields: `ghost` (ghost-button label, default "Not yet") and `backdrop` (value a backdrop tap resolves to, default false). New `agcBackdrop()`; the overlay-backdrop handler routes through it. The §154 submit panel is unchanged and was re-verified (ghost "Not yet", backdrop = cancel). Only the phone's local draft is touched; filed cases, server data, rank and coins never are. Chrome-verified all three paths on localhost.

# 🟢 §159 — THE BACKUP WAS RUNNING. IT WAS JUST NOT SAVING BUILDER CASES. s69.

**§139.4's "the clerk is 21 days dark" WAS WRONG** — it read the owner's LOCAL `Hunt-backups\snapshots`, which had not been `git pull`ed since 6 Aug. The GitHub Action ran green every Monday (8/17-9/14). **Read `origin/main`, not the local folder.** The real gap stood: only archive `hunt:` records were taken. `backup.py` step 5 (commit `7130bc8`) now writes EVERY other `hunt:` key to `cases/hunt-CODE.json`, rebuilt each run. First run 19 Sep: **175 builder cases, 14.9 MB, none vanished.** Repo is private (verified 404/403 unauthenticated) — it now holds player photos, keep it private.

Also found: `hunt:784051` (Piggy's original, §130 "unrecoverable") IS on the server — 48 photos, 0 clues, 0 hints, not deeded, expires ~21 Nov. **Owner ruling: dead to him; leave it to expire.** `335785` remains the case; still needs clues before acceptance.

Owner rulings s69: home-screen install IS exercised routinely by the owner (§139.6 item 6 / §112 item 1 CLOSED). Badge cast shadow stays `.47` (CLOSED). Grid inset at 430 left as is (CLOSED).

# 🟢 §157 — CODE-REVIEW FIXES (self-review of the session's new code). `35l`, s68.

**Owner: "run code review with the engineers," then "fix." Two Code & Architecture seats traced this
session's own `§` changes; four defects fixed, two systems cleared.**

1. **§149 (the real one, curator/owner only).** `saveDraft` refuses to write the personal-draft slot
   during a Desk/curator build (`_draftIsFresh` false), but `startBuild`'s RESTORE side did not: it
   restored any draft unconditionally. `curatorCreateHunt` sets `_curatorBuildFlow` then calls
   `startBuild`, so a personal draft could reappear inside the Desk flow → filed as an archive
   (`desk=true`) case, wrong home, and the unconditional `clearDraft` in `_finishBuild` then wiped the
   personal draft. FIX: restore gated on `!_curatorBuildFlow && !_curatorEdit`; `_finishBuild` clearDraft
   gated on `!_curatorBuildFlow && !_builderEdit && !_curatorEdit`.
2. **§154 timed auto-file vs the open panel.** If the pocket-watch expired while "Present your findings?"
   was open, `autoFile → finishHunt(true)` filed and left the panel floating; tapping it re-ran finish
   (idempotent coin, but a stuck dialog + a duplicate `case_finished`). FIX: `autoFile` calls `agcNo()`
   if `_agResolve` is set (resolves the parked promise, closes the panel); `finishHunt` adds
   `if(State.locked) return;` right after the confirm resolves.
3. **§150 bundle gate.** Was `!ownsPack(codes[0])` (only the first of Vol I's three codes) → now
   `!ownsVolume(_bv.id)` (checks all). Cosmetic gate inconsistency, no wrong price.
4. **§156 counter init.** `clueCount` now called after `openTileEdit` prefills `#tile-clue` and after
   `openCrop` clears `#crop-clue`, so the counter isn't stale until the first keystroke.

REVIEWED CLEAN (no change): §152 atomic seat cap (race-safe on D1; offline fallback + resume correct;
`seat:%` in the sweep; no `seat:`/`seats:` collision) and §155 review ergonomics (confirm-all alignment,
out-of-order handling, stamp apply/update, marks path). §153 `isStandalone` inert and harmless.

# 🟢 §156 — CLUE CHARACTER COUNTER. `35k`, s68.

**Aesthetic #4, owner-approved.** The 12-char clue cap stays; it just stops feeling like a wall. Both
clue inputs — `#crop-clue` (the crop overlay) and `#tile-clue` (the edit overlay) — now carry a live
`N/12` counter (`.clue-count`, absolutely positioned in the field's right padding) that turns **oxblood**
at the limit, and at 12 the existing "+tap here to add hidden hint" toggle brightens to oxblood
(`.hint-toggle.at-limit`). `clueCount(input, which)` runs on `oninput`. No new copy — it reuses the
existing hint affordance. (An earlier draft added a long "A hidden hint holds 140" line; it overflowed on
a phone and was dropped — reuse the short existing line only.)

# 🟢 §155 — REVIEW ERGONOMICS: FULL-HEIGHT ✓/✗, CONFIRM-ALL, JUDGED STAMP. `35k`, s68.

**Aesthetic #2, from the live run + the AoG interface seat: a 50-tile hunt × many hunters = ~400 tiny
taps, and the ✓/✗ were 44×32.** Fixed three ways, no hiding.

- **Full-height verdicts.** `.vtoggle{align-self:stretch}` so the toggle column fills the row; `.vt{flex:1
  1 0;min-height:44px;width:52px}` so ✓ and ✗ split the tile's full height 50/50 (owner's exact
  direction). Comfortably above the 44px target.
- **"Confirm all remaining"** (`confirmAllRemaining()`, a ghost button above the rows) marks every
  FOUND-and-unjudged tile ✓ in one tap. **It never touches a NOT-FOUND tile** — tiles can be claimed out
  of order, so it keys off `findMap[t.id]` (a real submission) AND `marks[t.id]===undefined`. `rows[i]`
  aligns with `hunt.tiles[i]` because a row is built per tile (found or not).
- **Judged stamp, NOT hiding.** Owner ruling: don't hide confirmed tiles (that removes the ability to
  revise); stamp them instead. `.vthumb.jstamp::after` reuses the EXACT `.tile.found` SOLVED-stamp CSS
  (Special Elite, rotate −11deg, bordered, parchment) recoloured: `.jyes` = green "CONFIRMED", `.jno` =
  oxblood "RETURNED", text via `data-stamp`. Applied in `mark()` (revisable — the other verdict
  restamps), in `confirmAllRemaining()`, and in `openVerify`'s render loop for tiles already marked.
  Everything stays visible and changeable.

# 🟢 §154 — THE AGENCY'S OWN SUBMIT PANEL. `35j`, s68.

**Owner ruling after a live end-to-end run + the AoG interface/completion/platform seats: the hunt
submit/lock used the browser's native `confirm()` — grey box, "gahensley1.github.io says", worded as
loss, and it fired ONLY on a partial finish (a full 20/20 got no confirm at all).** Replaced with the
Agency's own overlay.

- **`ov-agconfirm`** — a new overlay modelled byte-for-byte on `ov-buy`'s classes (`.overlay.center` ›
  `.modal` › `.lic-eyebrow` › `.clue-text` › `.row2` › `.btn.btn-primary` + `.btn-ghost`). No invented
  CSS; inherits the real parchment/brass look.
- **`agencyConfirm({eyebrow,title,bodyHTML,cta,plain})` → `Promise<boolean>`.** Primary button = true;
  ghost ("Not yet") and a backdrop tap = false. The backdrop is routed through `agcNo()` in the
  overlay-backdrop handler (alongside `ov-dispatch`/`ov-praise`) so a stray tap resolves the promise
  and never orphans it. If `#ov-agconfirm` is ever missing, it falls back to `window.confirm(plain)` —
  a caller can never hang.
- **Fires on BOTH partial and full** (the `doneT<totalT` gate is gone); `!auto` still skips it for a
  timed auto-submit.
- **Copy** (owner-approved, centered, numerals; final wording is the owner's): eyebrow `CASE No. <code>`,
  title "Present your findings?" / "A clean sweep?", the count `<n> of <N> cracked.` on its own line,
  the mid line, and "Once filed, it is sealed." in **oxblood** (ceremony colour per Branding-Guidelines).
  CTA "File my findings" / "File the case". Cold (archive) cases swap the seal line.

VERIFY AFTER SHIP (Chrome, live): panel appears on submit; "File my findings" files; "Not yet" and a
backdrop tap cancel with no stuck state; full-board finish now also shows it.

# 🟢 §153 — `isStandalone()` INSTALL-DETECTION HELPER. staged, ships with `35j`, s68.

A pure detection primitive: `navigator.standalone` (iOS) OR the `display-mode` media query
(standalone/fullscreen/minimal-ui), fully wrapped, false-by-default. **Inert** — nothing calls it yet.
It is the foundation for the coming push-availability fix (when notifications are unavailable *because*
it's an iPhone browser tab, show "Add to Home Screen…" instead of silently no-op-ing). Shipped now so
that fix is a one-line wiring later.

# 🟢 §152 — SEAT-RACE HARD CAP. Worker v2.6.18 + client `35i`, s68.

**Owner: "this needs to be fixed, no bites should be happening." The Code seat's seat-claim race, fixed
properly.** The old join counted the complement on the CLIENT against `sub:` (submission) rows, so two
hunters could both pass the last-seat check — and a joined-but-unsubmitted hunter wasn't even counted, so
the cap was soft. Now a seat is a SHARED row, `seat:CODE:HID`, enforced ATOMICALLY on the Worker.

WORKER (v2.6.18, new `/claim-seat` route): one statement — `INSERT ... SELECT ... WHERE (SELECT COUNT(*)
... LIKE 'seat:CODE:%') < limit AND NOT EXISTS(...)`. D1 serialises writes on its single primary, so
simultaneous claims cannot both land past the limit. `resume:true` force-seats an existing member
(`INSERT OR IGNORE`) so a returning hunter is NEVER turned away, even if the complement was lowered under
them. `seat:%` added to the retention sweep so seat rows die with the case. Everything else byte-identical
to v2.6.17. Gitignored; owner deployed it himself via the Cloudflare dashboard — Claude never held Worker
access (SSA.1).

CLIENT (`35i`): `joinCase` calls `claimSeat(code, hid, limit, resume)`. A NEW seat is gated by the atomic
claim; `seated:false` shows the existing "full complement" message (verbatim, unchanged) and records the
turn-away. A RESUME fires `resume:true` fire-and-forget so legacy hunters get a seat row and are counted.
If the Worker is unreachable (offline), the claim returns null and the join falls back to the OLD local
seat, so offline joining still works.

PROVEN: unit-tested against real SQLite (10 racers / 3 seats → exactly 3; idempotent; resume never
blocked; per-case isolated), then LIVE-tested against the deployed Worker (8 simultaneous claims → exactly
3 seated, 5 refused; resume + idempotent confirmed; test seats cleaned up).

TRANSITION NOTE: a case already near-full under the old submission-count rule starts with 0 `seat:` rows;
they accrue as hunters join/resume. Briefly such a case could seat a few over its nominal limit until
existing members resume (each resume force-creates their seat). Self-corrects; no action needed.
...
names mean nothing to a newcomer.** Fix: the bundle button carries a plain-English subtitle, exactly
like the Charter button, and sits stacked beneath the Charter so the $1 step up reads by comparison.

`renderLicTiers` now emits a second button when `!hasLicence()` **and** the bundle volume isn't already
owned: `onclick="buyEverything()"`, title (owner verbatim) "The Inspector's Charter & The Almanac,
Volume I", subtitle "unlimited builds + 3 ready-made cases — $8.99". The bundle resolves to Almanac
Vol I (`bundle:true`, §148). Buying it grants the Charter + the volume, so `hasLicence()` goes true and
both buttons vanish. Only the bundle is surfaced here; single volumes still live on the cold-case detail
card. The screen opens on tapping **Build**, so "the Charter is the tool, the bundle is the tool + three
cases" lands in context. Shipped as `35h`, on top of the already-shipped §149 (`35g`).

---

# 🟢 §149 — BUILDER AUTOSAVE, AND THE EU-PUSH COMMENT. `35g` (SHIPPED, commit `ec65bee`), s68.

**Owner: "do all" of the engineer-seat findings. First two done here.**

## §149.1 A FRESH BUILD NO LONGER VANISHES
The completion seat's top finding: a builder can lose a whole half-built case — `State.build` lived only
in memory, with nothing but a `confirm()` between an abandoned tab and a lost board. Now a **fresh** build
(one with no code minted yet) is persisted and, on the next builder open, **restored silently**.

- **Store: IndexedDB, not localStorage.** A photo board runs to megabytes; localStorage would throw
  `QuotaExceededError` on exactly the big cases worth saving. IDB holds it. All best-effort with
  try/catch — if IDB is unavailable the app behaves exactly as before (no draft, no harm).
- **Fresh builds ONLY.** `_draftIsFresh()` = `State.build && !code && !_builderEdit && !_curatorEdit &&
  !_curatorBuildFlow`. An amendment, a Desk build or a curator edit already has a filed copy and must
  never read or write this slot. Restore is only reached through `startBuild()`, the fresh entry point.
- **When it saves:** `scheduleDraftSave()` (800 ms debounce) at the end of `renderBuild` (covers tile
  add/remove and any edit that re-renders), a 4 s interval while `#s-build` is visible (covers clue/hint
  text typed between renders), and `visibilitychange`/`pagehide` (the reap moment; best-effort as the tab dies).
- **When it clears:** `_finishBuild` once the case is recorded in `mine[]` (filed, too-big or not-landed —
  all are now in `mine[]`/replay, so the draft's job is done), and `confirmLeaveBuild` when the builder
  confirms the "board will be lost" leave (so that copy stays true — they really did abandon it).
- **Restore is SILENT** — the board just reappears. No prompt, so no invented Victorian copy. OPEN,
  owner's call: a "resume this case, or start fresh?" prompt would be friendlier but needs his verbatim wording.
- **VERIFY ON HIS MACHINE after ship:** start a build, add 2–3 photo tiles, reload the tab → the board
  returns; then file it and start again → empty. A green STATIC does not prove IDB round-trips in a real browser.

## §149.2 THE STALE EU-PUSH COMMENT
`sw.js` claimed web push was "unavailable in the EU under 17.4+." True for ~3 weeks in early 2024, then
Apple reversed it (Platform seat). Comment corrected so no future decision treats the EU as a dead zone.
Comment-only; no behaviour change, SW CACHE untouched.

---

# 🟢 §148 — PRICING: CHARTER $7.99, BUNDLE $8.99 (FIXED + FLAGGED). `35f`, s68.

**Owner ruling after the market seat's review:** Charter to **$7.99** (was $6.99); bundle **down** to
**$8.99** (was $10.99). The bundle was priced $2.51 OVER its own parts (Charter $6.99 + volume $1.49 =
$8.48) AND un-buyable (no volume flagged `bundle:true`, so `bundleVolume()` returned null). Now: parts at
the new prices = $9.48, bundle $8.99 = a real 49¢ saving and only $1 over the Charter alone for 3 cases.

Edits, all in `index.html`: `$6.99`→`$7.99` at the three shown spots (`purchaseLicence` cta, its
`purchaseGranted` line, and the `renderLicTiers` button); `BUNDLE_PRICE` `"10.99"`→`"8.99"`; Almanac
Volume I (`alm1`) gains `bundle:true`. Volumes stay $1.49 (3 cases each; 3 packs = $4.47). Seat block
stays $4.99/+25. Coins stay earned-only (§93 — never rewire to a failure state).

**⚠ THE BUNDLE STILL SHOWS TO NO ONE.** `buyEverything()` is defined but called from nowhere, and
`renderLicTiers` deliberately sells only the Charter (its own comment: "This screen sells the Charter and
nothing else"). So the $8.99 is corrected and `bundleVolume()` now resolves to Almanac Vol I, but a buyer
sees no bundle until a surface + owner copy is added. LIVE structure today = Charter + Volumes + Seats.
OPEN, owner's call: surface the bundle on the licence screen (needs his verbatim copy).

Prices are preview grants; real StoreKit / Play billing + receipt validation still owed for store day.
STATIC green, `35f`; FULL BATTERY OWED ON HIS MACHINE before ship.

---

# 🟢 §147 — THE TOUR-CIPHER GUARD, AND AN EXPERT CODE REVIEW. `35e`, s68.

**Owner: "make sure the tour does not run until the cipher window closes," then "can experts review the
code for errors or misalignments."** Both done in one build.

## §147.1 THE TOUR NOW WAITS FOR THE CIPHER WINDOW
The cipher window is `#cred-ov` (the credentials overlay, spinning cipher disk). A per-path defer existed
but only covered the enrollment flow — the **idle-50s timer** and the **tap-to-replay** both called
`runHomeTour()` directly and could run the tour OVER the cipher. Fix: one guard at the single choke point,
the top of `runHomeTour()` — if `#cred-ov` is not hidden, set `State._pendingTourAfterCred=true` and
return. ALL trigger paths now respect it, and the existing cred-ov close handler resumes the deferred tour.
Chrome-verified: cipher open -> no `.show`, no play burned, pending set; cipher closed -> tour runs.

## §147.2 THE REVIEW — TWO AGENTS, AND THE VERDICT
Two independent agents audited `index.html` for real bugs (state/event correctness; errors/dead-states/
assets). **The codebase is exceptionally hardened** — the documented bug classes (three-view `coldQ`,
orphan submission vs deleted case, read-before-async) are genuinely closed, and the portrait/`safeImgSrc`
class has no other instances. Four contained fixes shipped in `35e`:
1. **Dispatch/Praise backdrop bypass.** `ov-dispatch`/`ov-praise` were closed by the GENERIC backdrop
   handler, skipping `burnDispatch`/`burnPraise` — the teardown that flushes queued toasts and fades the
   card. A backdrop tap delayed the "watch is set" toast several seconds onto the wrong screen. Now those
   two route through their burn; `ov-crop`/`ov-name` stay excluded as before.
2. **`coldSample` retry dead-end.** The error read "try again" but the button was already removed. Now the
   error line carries a clickable `try again` that re-runs `coldSample(code)`.
3. **`shareCaseCode` unguarded `JSON.parse`** -> wrapped; a corrupt body no longer silently breaks Share.
4. **`adoptProfile` non-object parse** -> `if(!p||typeof p!=="object") return null;` (a `"null"` body
   parsed clean then threw on `p.name`).

## §147.3 REVIEW FINDINGS LEFT OPEN (owner's call, not shipped)
- 🔴 **Seat-capacity race in `joinCase` (TOCTOU).** Two hunters joining a near-full case at the same
  instant can both read "one seat left" and both seat, exceeding the paid complement. A true fix is a
  SERVER-SIDE atomic seat-claim in the Worker (a `seatcount:` compare-and-set), not a client edit. Real,
  logged, not urgent.
- **`askName` single-resolver slot** — a re-entrant call would orphan the prior promise. Latent; today's
  awaited single-threaded call sites do not trigger it.
...
**🔴 §146.3 REVERTED AGAIN AT `35d` — BACK TO THE ORIGINAL.** The owner was not happy with the
black-on-white cutout either (legible, but the look was not right for him). Restored the ORIGINAL rendered
sign byte-exact. **PUB-SIGN ART IS SETTLED: THE ORIGINAL `67436406c2.webp` STAYS. Do not swap it again
without a clear owner request — two candidates were tried and reverted (§146.1, this).** The original saved
copy lives at `outputs/app-pubsign-current.webp` in-session; if lost, it is in any commit at or before
`e1146dad`.

**🟢 §146.2 RESOLVED AT `35c` — BLACK-ON-WHITE, CUT OUT.** The winning sign: a weathered black-on-white
enamel street nameplate (peak Victorian-London, the noise intact) — and the legibility problem is GONE at
**12.59:1** vs the gold's 4.3:1. It was CUT OUT so it sits as an object, not a photo box: the photo's white
background was flood-filled to transparent FROM THE BORDER, which the continuous black frame stops, so the
enclosed white sign face survives (the §128 enclosed-void pattern). Rounded transparent corners match the
original. Verified in the real tour: 360×96, transparent background, parchment through the corners, clean
edge, no halo. **The white sits naturally on the parchment — the palette worry was overblown.**

**🔴 §146.1 REVERTED AT `35b`.** The weathered brass sign shipped in `35a` and **did not read on the
phone** — the gold-on-green measured the lowest of the three candidates (4.3:1) and Claude had flagged it
as the one that would wash out in daylight; the device confirmed it. The owner reverted. Original sign
restored byte-exact, SW cache bumped again. **THE LESSON THAT OUTRANKS "FIT THE WORLD": a home-screen sign
used OUTDOORS ON A PHONE must clear contrast on the DEVICE first — the yellow candidate (4.53:1) was the
legible one and remains the option if the weathered look is wanted with phone-readable letters.** A
desktop screenshot and a contrast number are necessary but not sufficient; the owner's phone is the judge.
...
Continues §142. The **32 images held as JS `const` data-URIs** (COIN_*, PORTRAIT_* ×14, BADGE_MAC,
SEAL_IMG(S), PLQ_COLD, PAW_INK) were decoded to `assets/img/<contenthash>` and their inline URIs swapped
for `./assets/img/…` file URLs. **Zero embedded base64 images remain** in `index.html` (the only two
`data:image` strings left are CODE — the `safeImgSrc` validator and a comment, which police HUNTER
photos, not chrome art). Document **3.14 → 2.45 MB** (−690 KB); base64→binary trims the tax again.
CACHE bumped `shco-v3`→`shco-v4`.

## §143.1 THE RUNNING TALLY
`index.html` opened s68 at **4.40 MB** and is now **2.45 MB** — nearly halved, look untouched. What is
left: ~1.7 MB of code + the ~973 KB gazetteer (`GAZ5_BLOB` + `CITY2_BLOB`, still inline) + a 71 KB font.

## §143.2 🔴 WHY THE CANVAS COMPOSITES ARE SAFE BY GUARANTEE, NOT LUCK
The badge, coins and portraits are drawn to `<canvas>` (and BADGE_MAC into an SVG `<image href>`). Every
such draw first does `await new Promise(function(ok){ const i=new Image(); i.onload=()=>ok(i); i.src=URL; })`
and only then `drawImage`. To the browser a same-origin **file-URL** Image is identical to a data-URI
Image: `onload` fires for both, `drawImage` works for both, and `toDataURL` works for both **because the
assets are same-origin — no canvas taint**. The awaited-onload pattern IS the correct way to draw a
file-URL image; the only difference is a few ms of first-load latency the `await` already absorbs. There
is no failure mechanism. Verified in source across every draw site; the `toDataURL` calls touch only the
hunter's own cropped photo.

## §143.3 VERIFIED / NOT VERIFIED — HONEST
VERIFIED in Chrome on the real build: buildmark `34y`, **zero broken images across all 9 screens**, clean
boot (control-marker proven), all `<img>`/CSS art from files. NOT visually triggered: a *composited badge*
— the mint flow resisted scripted input and BEHAVIOUR's `test_renders` boots but does not force a
composite. The canvas path is sound by §143.2; the belt-and-suspenders visual is the owner minting a
badge on the build (his server is up) or his eye after ship. **Correctness is not in doubt; the specific
pixel was not photographed.**

## §143.4 PHASE 3 REMAINS (the big, riskier one)
The gazetteer (~973 KB) is the last large lever and the only one that is NOT a mechanical swap: both blobs
decode SYNCHRONOUSLY at boot and 12 geo functions read them, so moving them to a file makes boot async and
needs a ready-guard against a geo action firing before load. None of those functions run at boot, so a
non-blocking preload + guard is feasible — but it is real work with a race to test, not a src swap.
